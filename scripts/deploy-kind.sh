...
if ! kind get clusters | grep -q "^${CLUSTER_NAME}$"; then
    ...
    echo "Create one with: kind create cluster --name ${CLUSTER_NAME} --config kind-config.yaml"
    echo ""
    echo "Note: The kind-config.yaml mounts the models directory into the kind node"
    exit 1
fi
...
kind load docker-image bridge-rt-logic:latest --name "${CLUSTER_NAME}"
